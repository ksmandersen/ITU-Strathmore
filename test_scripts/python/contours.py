...
if __name__ == '__main__':
    ...
    #img = cv2.loadimage('shapes.jpg')
    im = cv2.imread('shapes2.jpg')
    ...
    contours2, hierarchy2 = cv2.findContours(thresh,cv2.RETR_CCOMP,cv2.CHAIN_APPROX_TC89_KCOS)
    #contours2, hierarchy2 = cv2.findContours(thresh,cv2.RETR_TREE,cv2.CHAIN_APPROX_TC89_KCOS)
    cv2.drawContours(im,contours2,-1,(0,255,0),3)
    cnt = contours2[0]
    ...
    # img = make_image()
    # h, w = img.shape[:2]
    # contours0, hierarchy = cv2.findContours( img, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
